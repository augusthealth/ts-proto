...
function isTransientField(fieldDesc: FieldDescriptorProto, meta: TransientExtensionMeta | undefined): boolean {
  ...
  return permissionBuffers.some((buf) => {
    ...
    // ts-proto-descriptors stores unknown fields as [length_varint, message_bytes...],
    // so read the length prefix first to find the message boundary.
    const messageLen = reader.uint32();
